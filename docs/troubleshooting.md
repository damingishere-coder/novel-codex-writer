...
1. 确认 Node.js 与 npm 已安装。
2. 在仓库根目录重新双击 `启动网页.bat`。
3. 检查 5174 端口是否被其他程序占用。
4. 查看 `Novel-Codex-Writer/.runtime/` 中的本机日志；提交 Issue 前先删除个人路径。

## 第一次启动很慢

第一次启动通常需要安装前端依赖，速度取决于网络和电脑性能。

不要在安装过程中反复点击启动脚本。先查看本机日志，并在页面可用后打开“启动预检”。
...
- 操作系统版本
- Node.js、npm 与 Python 版本
- 如果使用 Docker 备用模式，再附 Docker Desktop 版本
- 使用的仓库提交或版本
- 复现步骤
- 预期结果和实际结果
- 脱敏后的错误信息
