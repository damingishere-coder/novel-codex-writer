    ...
    <div className="workbench-shell">
      <header className="app-toolbar">
        ...
        <div className="history-buttons">
          <button className="icon-button" aria-label="返回上一份文档" title="返回上一份文档" disabled={model.documentHistoryIndex <= 0} onClick={() => actions.navigateHistory(-1)}><ArrowLeft size={17} /></button>
          <button className="icon-button" aria-label="前进到下一份文档" title="前进到下一份文档" disabled={model.documentHistoryIndex < 0 || model.documentHistoryIndex >= model.documentHistoryLength - 1} onClick={() => actions.navigateHistory(1)}><ArrowRight size={17} /></button>
        </div>
        <div className="project-switcher" title={`当前作品：${model.activeProject?.name ?? "未选择作品"}`}>
          <BookOpen size={16} />
        ...
        <label className="global-search">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              if (event.target.value.trim()) actions.openLeftPane();
            }}
            onKeyDown={(event) => {
              if (event.key === "Escape") {
                actions.setQuery("");
        ...
        <button className="toolbar-button" onClick={actions.openNewDocument} disabled={!model.activeProjectId}><Plus size={17} />新建文档</button>
        <div className="mode-switch" aria-label="文档模式">
          {(["preview", "review", "edit"] as WorkspaceMode[]).map((item) => (
            <button key={item} className={model.mode === item ? "active" : ""} onClick={() => actions.setMode(item)}>
              {item === "preview" ? "预览" : item === "review" ? "审校" : "编辑"}
            </button>
          ))}
        </div>
        {!model.rightVisible ? <button className="icon-button" onClick={actions.openRightPane} title="显示审校栏"><PanelRightOpen size={17} /></button> : null}
        <button className="icon-button" onClick={actions.toggleDark} title="切换明暗模式">{model.dark ? <Sun size={17} /> : <Moon size={17} />}</button>
      </header>

      <main
        ref={workbenchRef}
        className={cn("workbench-grid", model.leftCollapsed && "left-collapsed", !model.rightVisible && "right-collapsed")}
      ...
      >
        ...
        <section className="document-workspace">
          ...
          <div className="document-toolbar">
            ...
            <strong title={model.selectedPath}>{model.selectedEntry?.fileName ?? "尚未选择文档"}</strong>
            <ChevronDown size={14} />
            <div className="document-stats">
              <span>{model.dirty ? "未保存" : "已保存"}</span>
              <WordCount wordCount={model.wordCount} isChapter={model.selectedEntry?.groupId === "chapters"} />
              <span>Markdown</span>
            </div>
            {model.mode === "edit" ? (
              <button className="primary-button" onClick={actions.save} disabled={!model.dirty || model.saving}>
                {model.saving ? <LoaderCircle size={15} className="animate-spin" /> : <Save size={15} />}{model.saving ? "保存中" : "保存"}
              </button>
            ) : (
              <button className="soft-button" onClick={() => actions.setMode("edit")}><PencilLine size={15} />编辑</button>
            )}
            <button className="icon-button danger" onClick={actions.openDeleteDocument} disabled={!model.selectedPath} title="移到回收站"><Trash2 size={16} /></button>
          </div>
          ...
          <aside className="right-workbench">
            ...
            {model.rightTab === "recovery" ? (
              <RecoveryPanel
                ...
                onDocumentRestored={actions.documentRestored}
                onLibraryChanged={actions.libraryChanged}
                onNotice={actions.showNotice}
              />
            ) : null}
          </aside>
      ...
      {model.notice ? <div className="toast"><Check size={16} />{model.notice}</div> : null}
